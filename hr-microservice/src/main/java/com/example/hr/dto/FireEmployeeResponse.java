...
public class FireEmployeeResponse {
	...
	public static FireEmployeeResponse fromEmployee(Employee employee) {
		FireEmployeeResponse response = new FireEmployeeResponse();
		response.setIdentity(employee.getIdentity().getValue());
		response.setFirstName(employee.getFullname().getFirst());
		response.setLastName(employee.getFullname().getLast());
		response.setIban(employee.getIban().getValue());
		var employeeSalary = employee.getSalary();
		response.setSalary(employeeSalary.getValue());
		response.setCurrency(employeeSalary.getCurrency());
		response.setDepartment(employee.getDepartment());
		response.setFulltime(employee.isFulltime());
		response.setBirthYear(employee.getBirthYear().getValue());
		response.setPhoto(new String(employee.getPhoto().getValues()));
		return response;
	}
